...
export default {
  ...
  Mutation: {
    ...
    updatePassword: requiresAuth.createResolver(
      (parent, { oldPassword, newPassword }, { models, user: { _id }, SECRET, SECRET2 }) => {
        try {
          return models.User.findById({ _id }).then(async user => {
            ...
            if (!checkPassword) {
              return {
                ok: false,
                errors: [{ path: 'oldPassword', message: 'Contraseña incorrecta.' }]
              };
            }

            const salt = await bcrypt.genSalt(10);
            const passwordHash = await bcrypt.hash(newPassword, salt);
            await models.User.findByIdAndUpdate(_id, { password: passwordHash });
