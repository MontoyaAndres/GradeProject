...
export default {
  Query: {
    compareStudents: requiresAuth.createResolver(
      async (parent, { periodSelected, career }, { models }) => {
        const period1 = periodSelected[0]; // first period
        const period2 = periodSelected[1]; // second period

        const exists = [];

        const periodData1 = await models.Student.find({
          TipoSemestre: period1,
          CodigoPrograma: career
        });
        const periodData2 = await models.Student.find({
          TipoSemestre: period2,
          CodigoPrograma: career
        });
